...
<div>
	...
	<!--Taste-->
		<div style="background-image:url(/site_12_1/css/Element_Tops_Schatten.png);background-repeat:repeat-x;height:34px;width:34px;float:left"></div>
		<div style="background-image:url(/site_12_1/css/Element_Tops_links.png);width:4px;height:34px;float:left"></div>
		<div style="background-image:url(/site_12_1/css/Element_Tops_taste_Mitte.png);background-repeat:repeat-x;height:18px; min-width:200px; float:left; text-align:left; padding:8px; font-weight:bolder; text-decoration:underline">Sites zu Gruppen zuordnen</div>
		<div style="background-image:url(/site_12_1/css/Element_Tops_rechts.png);width:4px;height:34px;float:left"></div>
		<div style="clear:right"></div>
	<!--Taste Ende-->
	<!--Taste-->
...
<table border=0>
<tr>
    <td><? if($usergroup and $usergroup!="unselected") {echo "Verfügbare Sites:";} else {echo "Zuerst Usergruppe auswählen";}?> </td>
    <td>
    </td>
    <td>
...
$rights_avail=mysql_query("SELECT description FROM sites where ID not in (select sites_ID from user_sites_x_groups where groups_ID ='$usergroup')") or die (mysql_error());
$totalRows_rights_recordset_avail = mysql_num_rows($rights_avail);

$rights_assign=mysql_query("SELECT description FROM sites where ID in (select sites_ID from user_sites_x_groups where groups_ID ='$usergroup')") or die (mysql_error());
...
opt.taskMoveRight="add_site";
opt.taskMoveLeft="delete_site";
