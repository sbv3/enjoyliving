...
function moveSelectedOptions(from,to) {
	...
	for (var i=0; i<from.options.length; i++) {
		...
		if (o.selected) {
			...
			var usergroup=arguments[5];
			var url="Admin_table_result.php";
			var absender=arguments[0].name;
			var task=arguments[4];
			url=url+"?task="+task+"&value="+URLencode(to.options[index].value)+"&usergroupid="+usergroup;
			display_data(url);
...
function OT_transferLeft() { moveSelectedOptions(this.right,this.left,this.autoSort,this.staticOptionRegex,this.taskMoveLeft,this.usergroup); this.update(); }
function OT_transferRight() { moveSelectedOptions(this.left,this.right,this.autoSort,this.staticOptionRegex,this.taskMoveRight,this.usergroup); this.update(); }
