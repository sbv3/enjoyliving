...
{} else{
	echo "<meta HTTP-EQUIV=\"REFRESH\" content=\"0; url=$href_root/site_12_1/admin/papa/menu.php?men_id=$back_menu_id\">";
	exit;
}
...
?>
<script>
function test_sites(site_usage_counter)
{
	if(site_usage_counter>1)
	{
		if(confirm("Diese Seite wird vonmehreren Magazinen genutzt! Sind Sie sicher?"))
		{
			return true;
		} else 
		{
...
if (!isset($_SESSION['microtime']) or $_SESSION['microtime'] != $timestamp)
{
	...
	{
		$task="";
			new_element($menu_id,$element_layout_id,$sort);
	...
	if ($subcontent_anlegen=="1"){
		...
		if(mysql_num_rows($menu_params)>0)
			...
			$sort_params = mysql_query("select max(sort) from menu_hierarchy where site_id=$site_id and menu_id in ($sort_params)") or die ("couldn't get max sort");
...
(
	...
	WHERE element.menu_id=menu_hierarchy.menu_id 
		AND menu_hierarchy.menu_id=$active_menu_id
		AND menu_hierarchy.site_id!=$site_id
		AND element_layout.id=element.element_layout_id 
		AND element_layout.block_sharing>0
	GROUP BY element_layout.id, element_layout.id, menu_hierarchy.menu_id) AS blocked 
	ON blocked.id=element_layout.id
LEFT JOIN 
(
	...
	WHERE element.menu_id=menu_hierarchy.menu_id 
		...
		AND menu_hierarchy.site_id=$site_id
		AND element_layout.id=element.element_layout_id 
		AND element_layout.singular>0
	GROUP BY element_layout.id, element_layout.id, menu_hierarchy.menu_id) AS singular 
	ON singular.id=element_layout.id
WHERE TYPE NOT LIKE '%seitencontent%' AND element_layout.type NOT LIKE '%sponsorbanner%' AND active=1 and (available_to like '%,$site_id,%' or available_to is null)
GROUP BY description")or die("x4_sort");
$element_layout_num = mysql_num_rows($element_layout);

//Abfrage für die Sortierfunktion
$element_id_sort = mysql_query("SELECT element.id, element.sort FROM element, element_layout WHERE menu_id='$active_menu_id' and type not like '%seitencontent%' and element_layout.type not like '%sponsorbanner%' and element.element_layout_id=element_layout.id order by sort")or die("x4_sort");
$element_id_num_rows = mysql_num_rows($element_id_sort);

//Abfrage für die verwendung dieses Artikels
$site_usage_q=mysql_query("select description from sites, menu_hierarchy where menu_hierarchy.menu_id=$menu_id and menu_hierarchy.site_id=sites.id") or die("failed site-usage".mysql_error());
?>
...
<?
$site_usage_counter=0;
echo "<br>Dieser Artikel wird von folgenden Magazinen verwendet: <br>";
while ($site_usage_r=mysql_fetch_assoc($site_usage_q))
{echo "- ".$site_usage_r[description]."<br>";$site_usage_counter=$site_usage_counter+1;}


$whereami_description = mysql_query("SELECT description from menu where id =$menu_id");
$whereami_description =mysql_fetch_assoc($whereami_description );
$whereami_description =$whereami_description['description'];
echo "<br>".whereami("admin",$menu_id)." &raquo <a href='".find_googleurl($menu_id)."'>".$whereami_description."</a><br>";
?>

<body class="basislayout_body";>
<div style="clear:both; width:632px; padding-top:20px; float:none;" class="basislayout_contentcontainer1">

<div style="height:34px;width:34px;float:right"></div>
				...
				<div style="background-image:url(/site_12_1/css/Element_Tops_taste_Mitte.png);background-repeat:repeat-x;height:34px;float:right;">
					<div style="float:right; height:34px;">
						...
						<div style="float:right;">
							<a href="<? echo find_googleurl($active_menu_id); ?>" target="_new">
							<img src="/site_12_1/css/preview.gif" border="0" height="27"  vspace="3" title="Seitenvorschau"/>
							</a> 
						</div>
						<div style="float:right;">
							<? if($return_to_subcontent==1){?> 
							<a href="/site_12_1/admin/admin_V1.php?menu_id=<? echo $back_menu_id; ?>#subcontent_detail">
							<img src="/site_12_1/css/button_prev.png" border="0" title="Zurück zur Hauptseite"/>
							</a> 
							<? }?>
						</div>
					</div>
	...
	<div style="height:34px;width:34px;float:left"></div>
	<div style="background-image:url(/site_12_1/css/Element_Tops_links.png);width:4px;height:34px;float:left"></div>
	<div style="background-image:url(/site_12_1/css/Element_Tops_taste_Mitte.png);background-repeat:repeat-x;height:18px; min-width:200px; float:left; text-align:left; padding:8px">Auto-Teaser für <? echo $site_description;?></div>
	<div style="background-image:url(/site_12_1/css/Element_Tops_rechts.png);width:4px;height:34px;float:left"></div>
	<div style="clear:both"></div>

	<div style="background-color:#ffffff; border:dashed; border-width:2px; border-color:#Ff0000; padding-top:10px; padding-bottom:10px;">
		<? 
		include ("Admin_menu_teaser_default_prep.php");
		?>
	</div>
...
<br><br>
<div name="admin content" style="width:632px; float:left;" class="basislayout_contentcontainer1">
<?
...
while ($zeige=mysql_fetch_object($element_id)){
	...
	{	$scope="main";
		...
		<div style='width:100%;background-image:url(/site_12_1/css/Element_Tops_Schatten.png);height:34px;background-repeat:repeat-x;'>
				...
				<div style='background-image:url(/site_12_1/css/Element_Tops_taste_Mitte.png);background-repeat:repeat-x;height:34px;float:right;'>
					<div style="height:34px; float:left;">
						<? if($element_counter > 1){?><!-- SORT UP BUTTON bei allen, die nicht das oberste Element sind-->
							<div style=" float: left;font-size: 8px;position: relative;left: 26px;top: 21px;width: 0px;"> <? echo $element_sort_own ;?> </div>
							<form action="admin_V1.php?menu_id=<? echo $menu_id;?>" method="post">
					...
					<div style="height:34px; float:left;"><!-- Löschen button bei allen Elementen -->
						<form action="admin_V1.php?menu_id=<? echo $menu_id;?>" method="post" onSubmit="return test_sites(<? echo $site_usage_counter;?>)">
			...
			<div style="height:34px;width:34px;float:left"></div>
			<div style="background-image:url(/site_12_1/css/Element_Tops_links.png);width:4px;height:34px;float:left"></div>
			<div style="background-image:url(/site_12_1/css/Element_Tops_taste_Mitte.png);background-repeat:repeat-x;height:18px; min-width:200px; float:left; text-align:left; padding:8px"><? echo "$zeige->id: $zeiges->description"; ?> </div>
			<div style="background-image:url(/site_12_1/css/Element_Tops_rechts.png);width:4px;height:34px;float:left"></div>
			<div style="clear:both"></div>
		</div>
		...
		<div style='background-image:url(/site_12_1/css/Element_subcontent_taste_Mitte.png);background-repeat:repeat-x;height:38px;float:right;'>
			<div style="height:48px; float:left;">
				<div style="float:left; max-height:38px;">
					<form action="admin_V1.php?menu_id=<? echo $menu_id;?>" method="post" style="float:left">
						<div style="float:left;height:38x;width:auto; padding-top:8px; padding-bottom:8px; padding-left:8px; padding-right:8px;">
							<? 
							if($element_id_num_rows){
								mysql_data_seek($element_id_sort, $element_counter-1);
								$element_id_query = mysql_fetch_row($element_id_sort);
							}?>
							<select name="element_layout_id">
								<option value="Element auswählen">Element auswählen</option>
								<? if(mysql_num_rows($element_layout))
							...
							<input type="text" name="sort" size="4" value="<? echo $element_id_query[1]+10 ;?>">
							<input type="hidden" name="timestamp" value="<? echo $timestamp = microtime();?>">
