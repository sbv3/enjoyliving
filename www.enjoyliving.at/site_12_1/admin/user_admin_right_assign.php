...
<div>
	...
	<!--Taste-->
		<div style="background-image:url(/site_12_1/css/Element_Tops_Schatten.png);background-repeat:repeat-x;height:34px;width:34px;float:left"></div>
		<div style="background-image:url(/site_12_1/css/Element_Tops_links.png);width:4px;height:34px;float:left"></div>
		<div style="background-image:url(/site_12_1/css/Element_Tops_taste_Mitte.png);background-repeat:repeat-x;height:18px; min-width:200px; float:left; text-align:left; padding:8px; font-weight:bolder; text-decoration:underline">Rechte zu Gruppen zuordnen</div>
		<div style="background-image:url(/site_12_1/css/Element_Tops_rechts.png);width:4px;height:34px;float:left"></div>
		<div style="clear:right"></div>
	<!--Taste Ende-->
	<!--Taste-->
...
<table border=0>
<tr>
    <td><? if($usergroup and $usergroup!="unselected") {echo "Verfügbare Rechte:";} else {echo "Zuerst Usergruppe auswählen";}?> </td>
    <td>
    </td>
    <td>
...
$rights_avail=mysql_query("SELECT description FROM user_rights where ID not in (select rights_ID from user_groups_x_rights where groups_ID ='$usergroup') order by description") or die (mysql_error());
$totalRows_rights_recordset_avail = mysql_num_rows($rights_avail);

$rights_assign=mysql_query("SELECT description FROM user_rights where ID in (select rights_ID from user_groups_x_rights where groups_ID ='$usergroup') order by description") or die (mysql_error());
...
opt.taskMoveRight="add_right";
opt.taskMoveLeft="delete_right";
