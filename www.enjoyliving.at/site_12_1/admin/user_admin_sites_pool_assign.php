...
<div>
	...
	<!--Taste-->
		<div style="background-image:url(/site_12_1/css/Element_Tops_Schatten.png);background-repeat:repeat-x;height:34px;width:34px;float:left"></div>
		<div style="background-image:url(/site_12_1/css/Element_Tops_links.png);width:4px;height:34px;float:left"></div>
		<div style="background-image:url(/site_12_1/css/Element_Tops_taste_Mitte.png);background-repeat:repeat-x;height:18px; min-width:200px; float:left; text-align:left; padding:8px; font-weight:bolder; text-decoration:underline">Usergruppen Site-Pools zuordnen</div>
		<div style="background-image:url(/site_12_1/css/Element_Tops_rechts.png);width:4px;height:34px;float:left"></div>
		<div style="clear:right"></div>
	<!--Taste Ende-->
	<!--Taste-->
...
<table border=0>
<tr>
    <td><? if($usergroup and $usergroup!="unselected") {echo "Verfügbare Site-Pools:";} else {echo "Zuerst Usergruppe auswählen";}?> </td>
    <td>
    </td>
    <td>
...
$rights_avail=mysql_query("SELECT description FROM sites where ID not in (select site_id from user_groups_x_site_pool where group_id ='$usergroup')") or die (mysql_error());
$totalRows_rights_recordset_avail = mysql_num_rows($rights_avail);

$rights_assign=mysql_query("SELECT description FROM sites where ID in (select site_id from user_groups_x_site_pool where group_id ='$usergroup')") or die (mysql_error());
$totalRows_rights_recordset_assign = mysql_num_rows($rights_assign);
?>
    </td>
...
<form>
	...
	<select style="width:200px;" name="list1" multiple size=10 ondblclick="opt.transferRight()">
		<? if($totalRows_rights_recordset_avail > 0)
          {while($row = mysql_fetch_assoc($rights_avail))
	...
	<select style="width:200px;" name="list2" multiple size=10 ondblclick="opt.transferLeft()">
		<? if($totalRows_rights_recordset_assign > 0)
          {while($row = mysql_fetch_assoc($rights_assign))
     	echo "<option value=\"$row[description]\">$row[description]</option>";} 
		?>
	</select>
	</td>
...
<div id="table_result_error_messages" style="height:100px;"></div>
...
opt.taskMoveRight="add_site_pool";
opt.taskMoveLeft="delete_site_pool";
